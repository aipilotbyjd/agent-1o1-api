...
class HubspotCreateDealNode extends AppNode
{
    private const BASE_URL = 'https://api.hubapi.com';

    public function type(): string
    {
        return 'hubspot.create_deal';
    }
    ...
    public function execute(Run $run, array $config, array $context): array
    {
        ...
        $response = Http::timeout(15)->withToken($credential->data['token'] ?? '')->post(self::BASE_URL.'/crm/v3/objects/deals', ['properties' => $config['properties'] ?? []]);
        $ok = $response->successful();
        $this->recordMetric($run, $ok, $startedAt);
        if (! $ok) {
            throw new \RuntimeException('HubSpot create_deal failed: '.$response->body());
        }
