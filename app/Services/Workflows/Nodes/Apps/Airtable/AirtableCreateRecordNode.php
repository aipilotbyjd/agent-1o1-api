...
class AirtableCreateRecordNode extends AppNode
{
    ...
    public function execute(Run $run, array $config, array $context): array
    {
        ...
        $response = Http::timeout(15)->withToken($credential->data['token'] ?? '')->post(self::BASE_URL."/{$config['base_id']}/{$config['table']}", ['fields' => $config['fields'] ?? []]);
        $ok = $response->successful();
        $this->recordMetric($run, $ok, $startedAt);
        if (! $ok) {
            throw new \RuntimeException('Airtable create_record failed: '.$response->body());
        }
