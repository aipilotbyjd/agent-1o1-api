...
class MysqlUpdateNode extends AppNode
{
    ...
    {
        return 'MySQL: Update Rows';
    }

    public function description(): string
    {
        return 'Update rows in a MySQL table.';
    }
    ...
    {
        return ['type' => 'object', 'properties' => ['table' => ['type' => 'string'], 'where' => ['type' => 'object'], 'data' => ['type' => 'object']], 'required' => ['table', 'where', 'data']];
    }
    ...
    public function execute(Run $run, array $config, array $context): array
    {
        ...
        try {
            ...
            $affected = $db->table($config['table'])->where($config['where'] ?? [])->update($config['data'] ?? []);
