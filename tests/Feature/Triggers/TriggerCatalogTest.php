...
it('lists the trigger type catalog grouped by category', function () {
    ...
    expect($response->json('data.schedule'))->toHaveCount(5)
        ...
        ->and(collect($response->json('data'))->flatten(1)->count())->toBe(18);
});
...
it('creates a github push trigger whose filters gate the webhook', function () {
    ...
    $this->postJson("/api/v1/hooks/{$token}", ['ref' => 'refs/heads/main'], ['X-GitHub-Event' => 'push'])
        ->assertStatus(202);

    // Non-matching event is acknowledged but ignored.
    $this->postJson("/api/v1/hooks/{$token}", ['zen' => 'hi'], ['X-GitHub-Event' => 'ping'])
        ->assertOk()
        ->assertJsonPath('message', 'Event ignored by trigger filters.');
